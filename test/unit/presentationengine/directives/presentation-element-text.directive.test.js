...
(function() {
  describe('Test Presentation Element Text directive', function() {
    ...
    beforeEach(module('presentationengine', function($compileProvider){
      ...
      $compileProvider.directive('presentationStep',function(){
        var def = {
          ...
          template:'<!-- Presentation Step Element -->\n<div class="mock" ng-transclude></div>'
        };
        return def;
      });
    }));
    ...
    it('Try to compile a presentationElementText without parent presentationStep', function() {
      var element = $compile("<presentation-element-text></presentation-element-text>")($rootScope.$new(true));
      try {
        $rootScope.$digest();
      }
      catch(e) {
        expect(e.message.indexOf("Controller 'presentationStep', required by directive 'presentationElementText'") > -1).toBe(true);
      }
    });

    it('Try to compile a presentationStep', function() {
      var rootElement = $compile('<presentation-screen></presentation-screen>')(scope);
      var stepScope = scope.$new(true);
      var step = $compile("<presentation-step></presentation-step>")(stepScope);
      var elementScope = stepScope.$new(true);
      var element = $compile("<presentation-element-text></presentation-element-text>")(elementScope);
      step.append(element);
      rootElement.append(step);
      scope.$digest();
      var contents = element.contents();
      // Check that the compiled element contains the templated content
      expect(contents[0].nodeType).toEqual(element[0].COMMENT_NODE);
      expect(stepScope.elements.length).toEqual(1);
      expect(element.prop('tagName')).toEqual(angular.element('<presentation-element-text>').prop("tagName"));
